...
namespace Mytrip.Articles.Helpers
{
    public static class ArticlesHelper
    {
        ...
        public static HtmlString EditDeleteCategory(this HtmlHelper html, bool ShowEditDelete, bool ShowEditDeleteBlog, int id, string path)
        {
            var obj = new Object();
            ...
            if (ShowEditDelete || ShowEditDeleteBlog)
            {
                ...
                {
                    string a = GeneralMethods.ImgInput("/images/edite.png", "/Article/EditCategory/" + id + "/" + path, "rename",14) +
                      " "+  GeneralMethods.ImgInput("/images/delete.png", "/Article/DeleteCategory/" + id + "/" + path, "delete",14);
                    result.AppendLine(a);
                }
            }
            return new HtmlString(result.ToString());
        ...
        public static HtmlString ShowComments(this HtmlHelper html, mytrip_articles article, int returnId)
        {
            ...
            if (!blog && isUserHasRights(article, false))
            {
                if (!article.ApprovedComment)
                {
                    ...
                    imgOpen.MergeAttribute("src", "/Theme/" + ThemeSetting.theme() + "/images/approved.png");
                    ...
                    OpenComments.MergeAttribute("href", "/Article/OpenComments/" + article.ArticleId + "/" + returnId + "/" + article.Path);
                ...
                else
                {
                    ...
                    imgClose.MergeAttribute("src", "/Theme/" + ThemeSetting.theme() + "/images/noapproved.png");
                    ...
                    CloseComments.MergeAttribute("href", "/Article/CloseComments/" + article.ArticleId + "/" + returnId + "/" + article.Path);
            ...
            foreach (var comment in article.mytrip_articlescomments.OrderBy(x => x.CreateDate))
            {
                ...
                if ((!article.ModerateComments || comment.IsApproved) || (article.ModerateComments && hasRights))
                {
                    ...
                    divGravatar.InnerHtml = AvatarHelper.Avatar(html, comment.UserEmail, new { width = 50 }).ToString();
                    fieldset.InnerHtml = "<table class='comment'><tr><td class='first'>" + legend.ToString() + "<div class='comment'>" + comment.Body + "</div></td><td class='last'>" + divGravatar.ToString() + "</td></tr></table>";
        ...
        public static HtmlString ListCategories(this HtmlHelper html, IQueryable<mytrip_articlescategory> categories, string path)
        {
            ...
            foreach (var category in categories)
            {
                ...
                if (category.Blog && category.SubCategoryId == 0)
                {
                    ...
                    {
                        string a = GeneralMethods.ImgInput("/images/edite.png", "/Article/EditCategory/" + category.CategoryId + "/" + category.Path, "rename", 14) +
                      " " + GeneralMethods.ImgInput("/images/delete.png", "/Article/DeleteCategory/" + category.CategoryId + "/" + category.Path, "delete", 14);
                        td_first.InnerHtml = a + " ";
                    }
                    ...
                    string avatar = AvatarHelper.Avatar(html, category.UserEmail).ToString();
                    string avatar2 = string.Empty;
                    if (!String.IsNullOrEmpty(avatar))
                        avatar2 = "<div class=\"right\">" + avatar + "</div>";
                    td_first.InnerHtml = avatar2 + "<h3 class=\"hometitle\">" + td_first.InnerHtml + "</h3>";
                    TagBuilder profile = new TagBuilder("a");
                ...
                else
                {
                    ...
                    {
                        a_category.InnerHtml = "<br/>Topic: " + category.Title;
                        h4.InnerHtml += a_category.ToString();
                        string blogdetails = "<br/><span>" + ArticleLanguage.create_date + ": " + string.Format("{0:dd MMMM yyyy}", category.CreateDate) +
                            ", " + ArticleLanguage.views + ": " + category.Views + "," + ArticleLanguage.posts + ": " + count + "</span>";
                    ...
                    else { result.AppendLine(" | "+h4.ToString()); }
                }
            }
            return new HtmlString(result.ToString());
        }
        ...
        private static string Pager(this HtmlHelper html, int[] pagesIds, string staticText,string path)
        {
            ...
            if (pageIndex == 1)
            {
                result.AppendLine(GeneralMethods.Button(string.Empty, "1", true,"left"));
            }
            else
            {
                result.AppendLine(GeneralMethods.Button(Controller + GetIdx(pagesIds, 1) + "/" + path, "1", false,"left"));
            }
            ...
            if (pageIndex <= 4)
            {
                for (_page_Index = _pageIndex; pageTotal >= _page_Index; _page_Index++)
                {
                    ...
                    {
                        result.AppendLine(GeneralMethods.Button(string.Empty, _page_Index.ToString(), true,"left"));
                    }
                    else
                    {
                        result.AppendLine(GeneralMethods.Button(Controller + GetIdx(pagesIds, _page_Index) + "/" + path, _page_Index.ToString(), false,"left"));
                    }
            ...
            if (pageTotal >= 3)
            {
                ...
                {
                    result.AppendLine(GeneralMethods.Button(string.Empty, pageTotal.ToString(), true,"left"));
                }
                else
                {
                    result.AppendLine(GeneralMethods.Button(Controller + GetIdx(pagesIds, pageTotal) + "/" + path, pageTotal.ToString(), false,"left"));
                }
            }
            return result.ToString();
        }
                            ...
                            "<Playlist>" +
                                ...
                                "<Items>" +
                                    "<PlaylistItem>" +
                                        ...
                                        "<MediaSource>http://" + UsersSetting.applicationName();
            //value + 
