$(document).ready(function () {
    ...
    $('input.nameopenid').click(
        function () {
            ...
            $(id).slideDown('slow');
            $("#TitleOpenId").html(this.name);
            $('#_op').focus();
            var a = $('#_op').parent('div.textC');
            var b = $(a).parent('div.textbox');
            $(b).find('div.textC, div.textL, div.textR').addClass("ac");
        });
    $('input.nonameopenid').click(
        function () {
            openid = this.value;
            $(location).attr('href', '/Account/OpenIdLogin?openid=' + openid);
        });
    $('div.buttonIWR').click(
        function () {

            $(location).attr('href', '/Account/OpenIdLogin?openid='
