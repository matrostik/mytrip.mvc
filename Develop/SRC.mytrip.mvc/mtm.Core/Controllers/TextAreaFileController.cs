...
namespace mtm.Core.Controllers
{
    ...
    public class TextAreaFileController : ICoreController
    {
        ...
        public ActionResult Index(string id, string id2)
        {
            ...
            if (User.IsInRole(UsersSetting.roleChiefEditor()) && !id.Contains("()Content()UserFiles"))
                id = "()Content()UserFiles";
            else if (!User.IsInRole(UsersSetting.roleChiefEditor()) && !id.Contains("()Content()UserFiles()" + User.Identity.Name))
                id = "()Content()UserFiles()" + User.Identity.Name;
