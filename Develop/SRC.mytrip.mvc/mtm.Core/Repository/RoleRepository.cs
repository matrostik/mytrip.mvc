...
namespace mtm.Core.Repository
{
    ...
    internal class RoleRepository : RoleProvider
    {
        ...
        {
            return entities.mytrip_usersroles.Where(x => x.RoleName == roleName)
                .SelectMany(x => x.mytrip_users
                    .Where(y=>y.UserName.Contains(usernameToMatch)==true)
                    .Select(y=>y.UserName))
                    .ToArray();
        }
        ...
        {
            return entities.mytrip_usersroles
                .Select(x => x.RoleName)
                    .ToArray();
        }
        ...
        {
            return entities.mytrip_users.Where(x => x.UserName == username)
                .SelectMany(x => x.mytrip_usersroles
                    .Select(y => y.RoleName))
                    .ToArray();
        }
