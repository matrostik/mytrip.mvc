...
<%if (HttpContext.Current.User.IsInRole("admin"))
    ...
    &nbsp;&nbsp;<%=Mytrip_Mvc_Language.right_control_panel%></div><div class="right_content">
    <a href="<%=Url.Action("C", "A", new{a=2})%>">    
    <%=Mytrip_Mvc_Language.menu_site_adjustment%></a><br /><br />
    <%bool a = (bool)ViewData["abstract_email_approved"];
      if (a == true)
      { %>
     <a href="<%=Url.Action("L", "A")%>">
    <%=Mytrip_Mvc_Language.email_setting%></a><br /><br /><%} %>
   <a href="<%=Url.Action("D", "A", new{a=0, b=1,c=25,d="Users"})%>">
    <%=Mytrip_Mvc_Language.menu_management_users%></a><br /><br />
     <a href="<%=Url.Action("F", "F")%>">
    <%=Mytrip_Mvc_Language.menu_file_manager%></a><br /><br /></div>  
    <%}
         %>