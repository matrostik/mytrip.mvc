...
<!--/*   Mytrip.Mvc.Web.Linq2sql   Copyright ©  2009 Стюхин Олег Анатольевич   */-->
    <h2>
        <%=Mytrip_Mvc_Language.menu_register %></h2>
    <p>
    ...
    <div>
        <fieldset>
            <p>
                <label for="UserName">
                    <%=Mytrip_Mvc_Language.username%></label>
                <%= Html.TextBoxFor(m => m.UserName) %> 
                <%= Html.ValidationMessageFor(m => m.UserName) %>
            </p>
            <p>
                <label for="Email">
                    <%=Mytrip_Mvc_Language.email%></label>
                <%= Html.TextBoxFor(m => m.Email) %> 
                <%= Html.ValidationMessageFor(m => m.Email) %>
            </p>
            <p>
                <label for="Password">
                    <%=Mytrip_Mvc_Language.password%></label>
                <%= Html.Password("Password") %>
                <%= Html.ValidationMessageFor(m => m.Password) %>
            </p>
            <p>
                <label for="ConfirmPassword">
                    <%=Mytrip_Mvc_Language.confirm_password%></label>
                <%= Html.Password("ConfirmPassword") %>
                <%= Html.ValidationMessageFor(m => m.ConfirmPassword) %>
            </p>           
                       <div style="width: 200px"><label><%=Mytrip_Mvc_Language.verifying_phrase%></label>
