...
namespace Mytrip.Mvc.Model.Linq2sql.Artycles
{
    public class CommentRepository
    {
        ...
        {
            var abc = _db.mt_artycle_comment.Where(x => x.mt_artycle.Id == a);
            return abc.OrderBy(x => x.Id);
        }
        /// <summary>
        ...
        {
            var abc = _db.mt_artycle_comment.Where(x => x.AddedBy == HttpContext.Current.User.Identity.Name);
            return abc.OrderByDescending(x => x.AddedDate);
        }
        /// <summary>
        ...
        {
            var abc = _db.mt_artycle_comment.Where(x => x.AddedBy == HttpContext.Current.User.Identity.Name);
            return abc.OrderBy(x => x.AddedDate);
        }
        #endregion
