...
namespace mtm.Store.Helpers
{
    ...
    public static class MoneyHelpers
    {
        public static IDictionary<string, string> CultureMoney()
        {
            ...
            var core = _doc.Root.Elements("mtm.Store").Elements("add").FirstOrDefault(x => x.Attribute("name").Value == "Money");
        ...
        private static decimal GetRateMoney(string id)
        {
            ...
            XDocument _doc = XDocument.Load(_absolutDirectory);
            //money container
            var money = _doc.Root.Elements("mtm.Store")
                .Elements("add")
