...
namespace mtm.Core.Helpers
{
    ...
    public static class UserHelper
    {
        ...
        public static HtmlString LogonMenu(this HtmlHelper html)
        {
            ...
            if (!CoreSetting.Development()&&(MytripUser.UserInRole(UsersSetting.roleAdmin()) || MytripUser.UserInRole(UsersSetting.roleChiefEditor())))
            {
            ...
            if (!CoreSetting.Development() && UsersSetting.unlockVisibleLogon())
            {
                ...
                if (!String.IsNullOrEmpty(HttpContext.Current.User.Identity.Name))
                {
                    ...
                    result.AppendLine(LanguageHelper.LanguageMenu());
                        result.AppendLine(ThemeHelper.ThemeMenu());
        ...
        public static HtmlString AccordionSearch(this HtmlHelper html)
        {
            TagBuilder form = new TagBuilder("form");
            string url = (HttpContext.Current.Request.Path.ToString() == "/") ? "/Home/Index" : HttpContext.Current.Request.Path.ToString();
            form.MergeAttribute("action", string.Concat("/Home/Search?url=", url));
            ...
            return new HtmlString(GeneralMethods.Accordion(CoreLanguage.search, form.ToString()));
        }
        public static HtmlString AccordionLatestUser(this HtmlHelper html)
        {
            ...
            if (!ProfileSetting.unlockGravatar())
                result.AppendLine("</ul>");
            return new HtmlString(GeneralMethods.Accordion(CoreLanguage.RecentVisitors, result.ToString()));
        }
        /// <summary>
        ...
        public static HtmlString ProfileInfo(this HtmlHelper html,mytrip_usersprofile x)
        {
            if (!x.ProfileClose || (HttpContext.Current.User.Identity.IsAuthenticated && HttpContext.Current.User.Identity.Name == x.mytrip_users.UserName))
            {
                ...
                if (HttpContext.Current.User.Identity.IsAuthenticated && HttpContext.Current.User.Identity.Name == x.mytrip_users.UserName)
                    result.AppendLine("<div class=\"right\">" + GeneralMethods.ImgInput("/images/edite.png", "/Home/EditProfile/" + x.mytrip_users.UserName, "rename", 16) + "<a href='/Home/EditProfile/" + x.mytrip_users.UserName + "'>" + CoreLanguage.editProfile + "</a></div>");
