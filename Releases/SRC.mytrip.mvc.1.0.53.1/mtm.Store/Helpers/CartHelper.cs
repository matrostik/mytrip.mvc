...
namespace mtm.Store.Helpers
{
    public static class CartHelper
    {
        ...
        public static string MyCart()
        {
            ...
            string[] _cart = StoreCookies.Cart.Replace("][", "|").Replace("[", "").Replace("]", "").Split('|');
        ...
        {
            IStoreRepository istore = new IStoreRepository();
            //MtmCart = MtmCart.Replace("][", "|").Replace("[", "").Replace("]", "");
            string[] _cart = StoreCookies.Cart.Replace("][", "|").Replace("[", "").Replace("]", "").Split('|');
            StringBuilder result = new StringBuilder();
        ...
        {
            int _id = id ?? 0;
            if (_id > 0 && !StoreCookies.Cart.Contains(string.Format("_{0}_", _id)))
                StoreCookies.Cart += string.Format("[_{0}_1]", _id);
            IStoreRepository istore = new IStoreRepository();
            string[] _cart = StoreCookies.Cart.Replace("][", "|").Replace("[", "").Replace("]", "").Split('|');
            StringBuilder result = new StringBuilder();
            decimal totalprice = 0;
