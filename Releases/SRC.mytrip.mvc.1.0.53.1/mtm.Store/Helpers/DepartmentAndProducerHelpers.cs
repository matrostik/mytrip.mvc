...
namespace mtm.Store.Helpers
{
    public static class DepartmentAndProducerHelpers
    {
        ...
        public static HtmlString StoreTitleDepartmentAndProducer(this HtmlHelper html, TitleDepartmentModel x)
        {
            ...
            string producer = string.Format("{0}<h1 class=\"title\">{1}</h1>", StoreHelper.StoreAddCategoryAndProduct(x.createdepartment, x.count, x.subDepartmentId, x._search, x.producer), ModuleSetting.nameProducer());
            string _store = string.Format("{0}<h1 class=\"title\">{1}</h1>", StoreHelper.StoreAddCategoryAndProduct(x.createdepartment, x.count, x.subDepartmentId, x._search, x.producer), ModuleSetting.nameStore());
            if (x.count >= 0 || x._search)
            {
                producer = string.Format("{0}<h1 class=\"title\"><a href=\"/Store/Index/1/10/0/0/1/Producer\" >{1}</a></h1>", StoreHelper.StoreAddCategoryAndProduct(x.createdepartment, x.count, x.subDepartmentId, x._search, x.producer), ModuleSetting.nameProducer());
                _store = string.Format("{0}<h1 class=\"title\"><a href=\"/Store/Index/1/10/0/0/1/Department\" >{1}</a></h1>", StoreHelper.StoreAddCategoryAndProduct(x.createdepartment, x.count, x.subDepartmentId, x._search, x.producer), ModuleSetting.nameStore());
            }
            if (x.producer)
