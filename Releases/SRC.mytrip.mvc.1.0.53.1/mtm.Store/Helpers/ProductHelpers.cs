...
namespace mtm.Store.Helpers
{
    public static class ProductHelpers
    {
            ...
            bool producer, bool DepartmentAndProducer, bool DepartmentAndProducer2)
        {
            ...
            foreach (var article in department)
            {
                ...
                if (article.ViewVotes)
                    votes = GeneralMethods.CoreRating(article.ViewVotes, false, (double)article.TotalVotes, -1) + "<br/>";
                ...
                if (StoreCookies.AddAcount != null && StoreCookies.AddAcount.Length >0 && StoreCookies.AddAcount != "0")
                    mycart = GeneralMethods.Button("/Store/AddPosition/" + StoreCookies.AddAcount + "/" + article.ProductId, StoreLanguage.addaccount, false, "right"); 

                string checkeds = StoreCookies.Comparison.Contains(string.Format("[{0}]", article.ProductId)) ? "checked='checked'" : string.Empty;
                string cart = StoreCookies.Cart.Contains(string.Format("_{0}_", article.ProductId)) ? "checked='checked'" : string.Empty;
                ...
                string storecart = "";
                if (article.Price > 0)
                    storecart = "<br/><input " + cart + " type='checkbox' id='cart" + article.ProductId + "' value='_" + article.ProductId
                    + "_1' class='cart' /> <label for='cart" + article.ProductId + "'>" + string.Format(StoreLanguage.cart, "</label><a href='/Store/Cart/" + article.ProductId + "'>" + StoreLanguage.cart2 + "</a>");
                string comparision = "<br/><input " + checkeds + " type='checkbox' id='compare" + article.ProductId + "' value='" + article.ProductId
                    + "' class='comparision' /> <label for='compar" + article.ProductId + "'>" + string.Format(StoreLanguage.compare, "</label><a href='/Store/View/0/Comparision'>" + StoreLanguage.compare2 + "</a>");
            ...
            bool producer, bool DepartmentAndProducer, bool DepartmentAndProducer2, bool compare)
        {
            ...
            foreach (var article in department)
            {
                ...
                if (article.NamberCatalog != null)
                    nambercatalog = " (" + article.NamberCatalog + ")";
                string storecart = "";
                string body = GeneralMethods.RemoveLinkContent(article.Body, 1000);
                if (article.Price > 0)
                    storecart = "<br/><input " + cart + " type='checkbox' id='cart" + article.ProductId + "' value='_" + article.ProductId
                    + "_1' class='cart' /> <label for='cart" + article.ProductId + "'>" + string.Format(StoreLanguage.cart, "</label><a href='/Store/Cart/" + article.ProductId + "'>" + StoreLanguage.cart2 + "</a>");
                string comparision = "<br/><input " + checkeds + " type='checkbox' id='compar" + article.ProductId + "' value='" + article.ProductId
                    + "' class='comparision' /> <label for='compar" + article.ProductId + "'>" + string.Format(StoreLanguage.compare, "</label><a href='/Store/View/0/Comparision'>" + StoreLanguage.compare2 + "</a>");
                ...
                _content += "<b><a href=\"/Store/View/" + article.ProductId + "/" + article.Path + "\" class=\"hometitle\" >" +
                   article.Title + nambercatalog + "</a></b><br/>" + body + "</td></tr></table>" + mycart + votes + prise + packing + countProduct + _date + departmentlink
        ...
        public static HtmlString ViewProduct(this HtmlHelper html, mytrip_storeproduct x)
        {
            ...
            string checkeds = StoreCookies.Comparison.Contains(string.Format("[{0}]", x.ProductId)) ? "checked='checked'" : string.Empty;
            string cart = StoreCookies.Cart.Contains(string.Format("_{0}_", x.ProductId)) ? "checked='checked'" : string.Empty;
            string storecart = "";
            if (x.Price > 0)
                storecart = "<br/><input " + cart + " type='checkbox' id='cart" + x.ProductId + "' value='_" + x.ProductId
                   + "_1' class='cart'/> <label for='cart" + x.ProductId + "'>" + string.Format(StoreLanguage.cart, "</label><a href='/Store/Cart/" + x.ProductId + "'>" + StoreLanguage.cart2 + "</a>");
            string comparision = "<br/><input " + checkeds + " type='checkbox' id='compar" + x.ProductId + "' value='" + x.ProductId
                + "' class='comparision' /> <label for='compar" + x.ProductId + "'>" + string.Format(StoreLanguage.compare, "</label><a href='/Store/View/0/Comparision'>" + StoreLanguage.compare2 + "</a>");
            string saleprice = "<br/>";
            int sale1 = 0;
            DateTime saledate = DateTime.Now;
            //Отображение скидки для отдела
            ...
            string votes = string.Empty;
            if (x.ViewVotes)
                votes = GeneralMethods.CoreRating(x.ViewVotes, true, (double)x.TotalVotes, x.mytrip_storevotes.Count()) + "<br/>";
            string mycart = "";
            if (x.Price > 0)
            ...
            if (StoreCookies.AddAcount != null && StoreCookies.AddAcount.Length >0 && StoreCookies.AddAcount != "0")
                mycart = GeneralMethods.Button("/Store/AddPosition/" + StoreCookies.AddAcount + "/" + x.ProductId, StoreLanguage.addaccount, false, "right");
        ...
        private static string FotoOptions(mytrip_storeproduct x, out int countfoto)
        {
            ...
            foreach (var _x in _result)
            {
                if (!_x.Name.Contains("product"))
                {
                    ...
                    result.Append("<img src='/Content/Store/Product/" + x.ProductId + "/" + _x.Name + "' alt='"+_x.Name+x.Title+"' style='width:" + ModuleSetting.widthImgProduct() + "px'/></td>");
                    if (__count % 3 == 0)
                    {
                        tr = false;
                        result.Append("</tr>");
        ...
        public static string ViewProduct(mytrip_storeproduct x, int content, int imgwidth)
        {
            ...
            if (x.NamberCatalog != null)
                nambercatalog = " (" + x.NamberCatalog + ")";
            string votes = string.Empty;
            string body = GeneralMethods.RemoveLinkContent(x.Body, content);
            if (x.ViewVotes)
            ...
            if (StoreCookies.AddAcount != null && StoreCookies.AddAcount.Length >0 && StoreCookies.AddAcount != "0")
                mycart = GeneralMethods.Button("/Store/AddPosition/" + StoreCookies.AddAcount + "/" + x.ProductId, StoreLanguage.addaccount, false, "left");
            string _content = StoreHelper.GetImageProduct(x.ProductId, imgwidth) +
                ...
                + body + "</td></tr><tr><td>" + mycart + "<div id='votes' class='right'>" + votes +
                /*"</div><div class='right'>" + */prise + departmentlink + "</div></td></tr></table>";
            return _content;
        }
    }
}
