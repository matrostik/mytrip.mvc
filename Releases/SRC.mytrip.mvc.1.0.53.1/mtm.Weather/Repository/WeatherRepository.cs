...
namespace mtm.Weather.Repository
{
    public class WeatherRepository
    {
        ...
        public mytrip_weather CreateWeather(ManagerModel m)
        {
            ...
            mytrip_weather x = new mytrip_weather
            {
                ...
                UserName = username,
                VisibleInformer=m.VisibleInformer
            };
            entities.mytrip_weather.AddObject(x);
            entities.SaveChanges();
            return x;
        }
        public void EditWeather(ManagerModel m)
        {
            mytrip_weather x = GetOnlyWeather(m.WeatherId);
            x.Title = m.Title;
            x.AllCulture = m.AllCulture; 
            x.VisibleInformer = m.VisibleInformer;
            x.UrlXml = m.UrlXml;
            entities.SaveChanges();
        }
