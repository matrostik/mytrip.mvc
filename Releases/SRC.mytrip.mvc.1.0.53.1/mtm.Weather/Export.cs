...
namespace mtm.Weather
{
    ...
    {
        public static HtmlString mtmWeatherExportAccordionWeather(this HtmlHelper html)
        {
            if (ModuleSetting.unlockWeather())
            {
                ...
                foreach (var item in ar.GetWeatherForAccardion(culture))
                {
                    ...
                    _result.AppendLine("<b>" + item.Title + "</b><div class=\"div_border\" style=\"border-left:0;border-right:0;\"></div>");
                    ...
                    if (Weather != null)
                    {
                        foreach (var _Weather in Weather)
                        {
                            ...
                            if (hour == 0 || hour == 2)
                            {
                                ...
                                string img = "<img src=\"/Weather/Image/_" + hour + "_" + phenom + "\" alt=\"phenom"+hour+phenom+"\" style=\"width:22px;position:relative;float:left;\"/>";
                                _result.AppendLine(img + " " + _hour + " " + _termmin + termmin + "&deg;C..." + _termmax + termmax + "&deg;C");
                                _result.AppendLine("</td></tr>");
                            }
                        }
                    }
                ...
                return new HtmlString(GeneralMethods.Accordion(
                    ...
                    ModuleSetting.partialNoAccordion(), html, cssclacc, ModuleSetting.NameWeatherPage(), _result.ToString() + Weatherlink));
            }
            else { return null; }
        }
