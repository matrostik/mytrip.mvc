...
namespace mtm.Core.Repository
{
    ...
    internal class MembershipRepository : MembershipProvider
    {
        ...
        {
            if(UsersSetting.AutoGenerateUserName())
                username=mtAutogenerateUserName(username);
        ...
        public override bool ValidateUser(string username, string password)
        {
            ...
            {
                var user =(UsersSetting.EmailasUsername())?mtGetMembershipByUserEmail(username): mtGetMembershipByUserName(username);
        ...
        internal string mtAutogenerateUserName(string username)
        {

            string result = username;
            int a = 0;
            while (mtGetUserByUserName(result) != null)
            {
                a++;
                result = username+a;
            }
        ...
        internal bool mtCheckNullUserName(string username)
        {
            ...
            {
                string _users = entities.mytrip_usersmembership
                 .Where(x => x.Email == username).Select(x => x.Email).FirstOrDefault();
                if (_users != null && _users == username)
                {
                    result = true;
                }
        ...
        private mytrip_usersmembership mtGetMembershipByUserEmail(string email)
        {
            return entities.mytrip_usersmembership.Include("mytrip_users").FirstOrDefault(x => x.Email == email);
        }
        public string mtGetUserNametoEmail(string email)
        {
            var a= entities.mytrip_usersmembership.Include("mytrip_users").FirstOrDefault(x => x.Email == email);
            if (a != null)
                return a.mytrip_users.UserName;
            else
                return string.Empty;
        }
        /// <summary>
