...
namespace mtm.Core.Settings
{
    ...
    public static class GeneralMethods
    {
        ...
        public static void MytripCacheRemove(string key, string _culture)
        {
            if (HttpContext.Current.Cache[key + _culture] != null)
                HttpContext.Current.Cache.Remove(key + _culture);
        }
        ...
        public static object MytripCache(string key, string element, string attribute, bool culture,string _culture, int? absolutSek, int? spanSek, CacheItemPriority priority)
        {
            if (HttpContext.Current.Cache[key + _culture] == null)
            {
                TimeSpan _spanSek = spanSek == null ? TimeSpan.Zero : TimeSpan.FromSeconds((int)spanSek);
                DateTime _absolutSek = absolutSek == null ? DateTime.MaxValue : DateTime.Now.AddSeconds((int)absolutSek);
                HttpContext.Current.Cache.Insert(key + _culture, MytripConfigurationValue(element, attribute,culture, _culture),
                null, _absolutSek, _spanSek, priority, null);
            }
            return HttpContext.Current.Cache[key + _culture];
        }
        public static object MytripCache(string key, string element, string attribute,string element2, int? absolutSek, int? spanSek, CacheItemPriority priority)
        {
            ...
            if (HttpContext.Current.Cache[key + _culture] == null)
            {
                TimeSpan _spanSek = spanSek == null ? TimeSpan.Zero : TimeSpan.FromSeconds((int)spanSek);
                DateTime _absolutSek = absolutSek == null ? DateTime.MaxValue : DateTime.Now.AddSeconds((int)absolutSek);
                HttpContext.Current.Cache.Insert(key + _culture, MytripConfigurationValue(element, attribute, element2),
                null, _absolutSek, _spanSek, priority, null);
            }
            return HttpContext.Current.Cache[key + _culture];
        }
        public static object MytripCache(string key, string element, string attribute, string element2,string _culture, int? absolutSek, int? spanSek, CacheItemPriority priority)
        {
           if (HttpContext.Current.Cache[key + _culture] == null)
            {
                TimeSpan _spanSek = spanSek == null ? TimeSpan.Zero : TimeSpan.FromSeconds((int)spanSek);
                DateTime _absolutSek = absolutSek == null ? DateTime.MaxValue : DateTime.Now.AddSeconds((int)absolutSek);
                HttpContext.Current.Cache.Insert(key + _culture, MytripConfigurationValue(element, attribute, element2,_culture),
                null, _absolutSek, _spanSek, priority, null);
            }
            return HttpContext.Current.Cache[key + _culture];
        }
        ...
        public static string MytripConfigurationValue(string element, string attribute,bool a, string culture)
        {
            string _absolutDirectory = GeneralMethods.MytripConfigurationDirectory();
            XDocument _doc = XDocument.Load(_absolutDirectory);
            string localization = LocalisationSetting.culture();
                var core = _doc.Root.Elements(element).Elements("add").FirstOrDefault(x => x.Attribute("name").Value == attribute);
                var _core = core.Elements("add").FirstOrDefault(x => x.Attribute("value").Value == culture);
                return _core.Attribute("name").Value;
        }
        public static string MytripConfigurationValue(string element, string attribute,string element2)
        {
            string _absolutDirectory = GeneralMethods.MytripConfigurationDirectory();
            XDocument _doc = XDocument.Load(_absolutDirectory);
                ...
                var core = _doc.Root.Elements(element).Elements("add").FirstOrDefault(x => x.Attribute("name").Value == attribute);
                var _core = core.Elements("add").FirstOrDefault(x => x.Attribute("value").Value == localization.ToLower());
                return _core.Element(element2).Value;
            
        }
        public static string MytripConfigurationValue(string element, string attribute, string element2,string culture)
        {
            string _absolutDirectory = GeneralMethods.MytripConfigurationDirectory();
            XDocument _doc = XDocument.Load(_absolutDirectory);
        ...
        /// <returns></returns>
        public static string Accordion(HtmlHelper html,string title, string content)
        {
            string[] a={ DecodingString(title).Replace("_", ""), title, content};
            return RenderViewToString(html, "Helpers/_accordion", a);
        }
        ...
        /// <param name="content">контент</param>
        /// <returns></returns>
        public static string Accordion(HtmlHelper html, bool accordion, string title, string content)
        {
            string[] a = { DecodingString(title).Replace("_", ""), title, content };
            return accordion
                ? RenderViewToString(html, "Helpers/_accordion", a)
                : RenderViewToString(html, "Helpers/_accordion_no", title);
        ...
        /// <param name="content">контент</param>
        /// <returns></returns>
        public static string Accordion(string file,HtmlHelper html, string title, string content)
        {
            string[] a = { DecodingString(title).Replace("_", ""), title, content };
            return RenderViewToString(html, file, a);
        }
        ...
        /// <param name="content">контент</param>
        /// <returns></returns>
        public static string Accordion(string file,string nofile, HtmlHelper html, bool accordion, string title, string content)
        {
            string[] a = { DecodingString(title).Replace("_", ""), title, content };
            return accordion
                ? RenderViewToString(html, file, a)
                : RenderViewToString(html, nofile, title);
            ...
            if (approvedvotes)
            {
                ...
                for (int rate = 0; rate < 5; rate++)
                {
                    ...
                    else
                    {
                        ...
                        input.MergeAttribute("alt", "rating "+(rate + 1));
                        result.AppendLine(input.ToString());
                    }
                }
            }
        ...
        /// <returns></returns>
        internal static string Menu(HtmlHelper html, string title, string content, bool visible, bool warning, bool menu, bool drop)
        {
            ...
            if (visible)
                _visible = "V";
            if(drop)
                    title = title.Replace("</a>", "<span class='drop'>&nbsp;&nbsp;</span></a>");
                string[] a = { _menu + _visible, title, content };
                return RenderViewToString(html, "Helpers/_menu_logon", a);
             
        }
        public static string Menu(string file,string wrapfile,HtmlHelper html, string title, string content, bool visible, bool warning, bool menu, bool drop)
        {
            string _menu = menu ? "menu" : "logon";
            string _visible = (warning && !visible) ? "W" : string.Empty;
            if (visible)
        ...
        internal static string Sitemap(HtmlHelper html, string title, string content)
        {

                string[] a = {title, content };
                return RenderViewToString(html, "Helpers/_sitemap", a);
        }
        ...
        public static string ImageForAbstract2(string image, int width)
        {
            if (image != null && image.Contains("src"))
            {
                image = image.Remove(0, image.IndexOf("src"));
        ...
        public static string RenderViewToString(this HtmlHelper html, string viewName)
        {
            ...
            System.Web.Mvc.Html.RenderPartialExtensions.RenderPartial(html, viewName);
            HttpContext.Current = oldContext;
            memWriter.Flush();
            return sb.ToString();
        }
        public static string RenderViewToString(this HtmlHelper html, string viewName, object data)
        {
            var sb = new StringBuilder();
            var memWriter = new StringWriter(sb);
            var fakeResponse = new HttpResponse(memWriter);
            ...
            var oldContext = HttpContext.Current;
            HttpContext.Current = fakeContext;
            System.Web.Mvc.Html.RenderPartialExtensions.RenderPartial(html, viewName, data);
            HttpContext.Current = oldContext;
            memWriter.Flush();
            return sb.ToString();
        }
