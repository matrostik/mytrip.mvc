    ...
    public class HomeController : ICoreController
    {
        ...
        public ActionResult Profile(string id, string id2)
        {
            ...
            if (model.UserProfile != null)
            {
                ...
                model.title = string.Format(CoreLanguage.profile, id);
                if (User.Identity.IsAuthenticated && User.Identity.Name == id)
                {
                    model.closeprofile = false;
                    model.title = CoreLanguage.my_profile;
                }
        ...
        public ActionResult EditProfile(string id)
        {
            if (HttpContext.User.Identity.Name == id)
            {
                ...
                if (GeoSetting.unlockGeo())
                    model.bodyonload = "GetMap();";
                string[] a = { "<a href='/Home/Profile/" + id + "'>" + CoreLanguage.my_profile + "</a>", CoreLanguage.editProfile };
                model.bread = a;
                return View(model);
        ...
        public ActionResult Page(int id)
        {
            ...
            if (x.SubPagesId == 0)
            {
                ...
                string[] a = { "<a href='/Home/Page/" + x.SubPagesId + "/" + x.mytrip_corepages2.Path + "'>" + x.mytrip_corepages2.Title + "</a>", x.Title };
                model.bread = a;
            }
