...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[mytrip_tourscountry]') AND type in (N'U'))
BEGIN
CREATE TABLE [dbo].[mytrip_tourscountry](
[AllCulture] [bit] NOT NULL,
[Body] [nvarchar](max) NULL,
...
[Title] [nvarchar](256) NOT NULL,
[UserName] [nvarchar](100) NOT NULL,
CONSTRAINT [PK_mytrip_tourscountry] PRIMARY KEY CLUSTERED 
(
[CountryId] ASC
)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY]
) ON [PRIMARY]
...
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[dbo].[FK_mytrip_tours_mytrip_tourscountry]') AND parent_object_id = OBJECT_ID(N'[dbo].[mytrip_tours]'))
ALTER TABLE [dbo].[mytrip_tours]  WITH CHECK ADD  CONSTRAINT [FK_mytrip_tours_mytrip_tourscountry] FOREIGN KEY([CountryId])
REFERENCES [dbo].[mytrip_tourscountry] ([CountryId])
...
ALTER TABLE [dbo].[mytrip_tours] CHECK CONSTRAINT [FK_mytrip_tours_mytrip_tourscountry]
