...
namespace mtm.Articles
{
    ...
    {
        #region Top Menu
        public static List<object> MenuArticle()
        {
        ...
        public static List<object> MenuBlog()
        {
        ...
        public static List<object> MenuCategory()
        {
            if (ModuleSetting.articles())
            {
                string[] urlpath = GeneralMethods.UrlDictionary(HttpContext.Current.Request.Path);
                ...
                foreach (var __item in ar.category.GetCategories(true, LocalisationSetting.culture()))
                {
                    ...
                    result.Add(ModuleSetting.partialMenuLogon());
                    result.Add(ModuleSetting.partialMenuLogonWrap());
                    result.Add(article.ToString());
                    result.Add(_result);
                    result.Add(tab_category);
                    result.Add(false);
                    result.Add(true);
                    result.Add(drop);
                }
        ...
        #region SideBar
        public static HtmlString mtmArticlesExportAccordionSearch(this HtmlHelper html)
        {
            ...
            return new HtmlString(GeneralMethods.Accordion(ModuleSetting.partialAccordion(),html,ModuleSetting.NameSearchPage(), form.ToString()));
        }
        public static HtmlString mtmArticlesExportAccordionArticle(this HtmlHelper html)
        {
            if (ModuleSetting.articles())
            {
                ...
                return new HtmlString(
                    GeneralMethods.Accordion(
                    ModuleSetting.partialAccordion(), 
                    ModuleSetting.partialNoAccordion(), html, cssclacc, _a.ToString(), ul.ToString()));
            }
            else { return null; }
        }
        public static HtmlString mtmArticlesExportAccordionBlogs(this HtmlHelper html)
        {
            if (ModuleSetting.blogs())
            {
                TagBuilder _a = new TagBuilder("a");
        ...
        public static HtmlString mtmArticlesExportAccordionCategory(this HtmlHelper html)
        {
            if (ModuleSetting.articles())
            {
                IArticleRepository ar = new IArticleRepository();
                ...
                foreach (var _item in ar.category.GetCategories(true, LocalisationSetting.culture()))
                {
                    ...
                    if (count > 0)
                        cssclacc = true;
                    result.AppendLine(GeneralMethods.Accordion(
                    ModuleSetting.partialAccordion(),
        ...
        public static HtmlString mtmArticlesExportAccordionTag(this HtmlHelper html)
        {
            if (ModuleSetting.articles() | ModuleSetting.blogs())
            {
                ...
                {
                    return new HtmlString(GeneralMethods.Accordion(
                    ModuleSetting.partialAccordion(),html, ModuleSetting.NameTagsPage(), _result.ToString()));
                }
                else { return null; }
            }
            else { return null; }
        }
        public static HtmlString mtmArticlesExportAccordionArticlesActivity(this HtmlHelper html)
        {
            ...
            return new HtmlString(GeneralMethods.Accordion(
                ...
                string.Format(ArticleLanguage.articles_popular_recent, ModuleSetting.NameArticlesPage()), form.ToString()));
        }
        public static HtmlString mtmArticlesExportAccordionBlogsActivity(this HtmlHelper html)
        {
            TagBuilder list = new TagBuilder("ul");
            list.AddCssClass("styled");
            IArticleRepository ar = new IArticleRepository();
            ...
            return new HtmlString(GeneralMethods.Accordion(
                ModuleSetting.partialAccordion(), html, 
                string.Format(ArticleLanguage.blogs_popular_recent, ModuleSetting.NameBlogsPage()), form.ToString()));
        }
        #endregion
        ...
        public static HtmlString Setting()
        {
            ...
            a2.MergeAttribute("href", "/Article/Seo");
            a2.InnerHtml = ArticleLanguage.article_seo;
            li2.InnerHtml = a2.ToString();
            HtmlString htmlresult = new HtmlString(li.ToString()+li2.ToString());
            return htmlresult;
            ...
            if (ModuleSetting.blogs())
            {
                    ...
                    && !isBlogger) || isBlogger)
                {
                    ...
                    foreach (var item in ar.category.GetBlogsByUser(HttpContext.Current.User.Identity.Name))
                    {
                        ...
                        {
                            a_blog.MergeAttribute("href", "/mtm/Language/" + item.Culture.ToLower() + "/(x)Article(x)Index(x)1(x)10(x)" + item.CategoryId + "(x)" + item.Path);
        ...
        public static HtmlString RssLink()
        {
            StringBuilder result = new StringBuilder();
            if (ModuleSetting.articles())
            {
                result.AppendLine("<link href=\"/RssArticle/RssArticles\" rel=\"alternate\" type=\"application/rss+xml\" title=\"" + ModuleSetting.NameArticlesPage() + "\" />");
                IArticleRepository ar = new IArticleRepository();
                foreach (var x in ar.category.GetCategories(true, LocalisationSetting.culture()))
                {
                    string _title = GeneralMethods.DecodingSearch(x.Title);
                    result.AppendLine("<link href=\"/RssArticle/RssArticlesInCategory/" + x.CategoryId + "/" + x.Path + "/" + _title + "\" rel=\"alternate\" type=\"application/rss+xml\" title=\"" + x.Title + "\" />");
                }
            }
            if (ModuleSetting.blogs())
                result.AppendLine("<link href=\"/RssArticle/RssBlogs\" rel=\"alternate\" type=\"application/rss+xml\" title=\"" + ModuleSetting.NameBlogsPage() + "\" />");
