...
namespace mtm.Articles.Repository
{
    public class CategoryRepository
    {
        ...
        /*  РАЗДЕЛ 2  */
        #region cache
        /// <summary>Получить все рубрики статей (в меню или недобавленные в меню) с начала 1->X
        /// </summary>
        /// <param name="forMenu">для меню или нет</param>
        /// <param name="culture">текущая культура</param>
        /// <returns></returns>
        internal IQueryable<mytrip_articlescategory> GetCategories(bool forMenu, string culture)
        {
            //if (HttpContext.Current.Cache["as_getcategories" + forMenu + culture] == null)
            //{
            //    TimeSpan _spanSek = TimeSpan.FromSeconds(36000);
            //    DateTime _absolutSek = DateTime.MaxValue;
                var x = _GetCategories(forMenu, culture);
                if (x == null)
                    return null;
                else return x;
            //    HttpContext.Current.Cache.Insert("as_getcategories" + forMenu + culture, x, null, _absolutSek, _spanSek, CacheItemPriority.High, null);
            //}
            //return HttpContext.Current.Cache["as_getcategories" + forMenu + culture] as IQueryable<mytrip_articlescategory>;

        }
        /// <summary>Получить все блоги по просмотрам
        /// </summary>
        /// <returns></returns>
        internal IQueryable<mytrip_articlescategory> GetBlogs(string culture)
        {
            //if (HttpContext.Current.Cache["as_getblogs" + culture] == null)
            //{
            //    TimeSpan _spanSek = TimeSpan.FromSeconds(36000);
            //    DateTime _absolutSek = DateTime.MaxValue;
                var x = _GetBlogs(culture);
                if (x == null)
                    return null;
                else return x;
            //    HttpContext.Current.Cache.Insert("as_getblogs" + culture, x, null, _absolutSek, _spanSek, CacheItemPriority.High, null);
            //}
            //return HttpContext.Current.Cache["as_getblogs" + culture] as IQueryable<mytrip_articlescategory>;

        }
        /// <summary>Получить одну рубрику по Id
        /// </summary>
        /// <param name="categoryId">номер рубрики</param>
        /// <returns></returns>
        internal mytrip_articlescategory GetCategory(int categoryId)
        {
            //if (HttpContext.Current.Cache["as_getcategory" + categoryId] == null)
            //{
            //    TimeSpan _spanSek = TimeSpan.FromSeconds(36000);
            //    DateTime _absolutSek = DateTime.MaxValue;
                var x = _GetCategory(categoryId);
                if (x == null)
                    return null;
                else return x;
            //    HttpContext.Current.Cache.Insert("as_getcategory" + categoryId, x, null, _absolutSek, _spanSek, CacheItemPriority.High, null);
            //}
            //return HttpContext.Current.Cache["as_getcategory" + categoryId] as mytrip_articlescategory;
        ...
        /// <returns></returns>
        internal mytrip_articlescategory CreateСategory(string title, bool separateBlock, 
            bool allCulture, string culture,string path,string keywords,string desc,string seotitle)
        {
            CreateСategoryZero();
            ...
            GeneralMethods.MytripCacheRemoveContains("as_getcategor");
            return x;
        }
        /// <summary> Создать подрубрику
        /// </summary>
        /// <param name="title">название</param>
        /// <param name="categoryId">номер рубрики</param>
        /// <param name="email">email пользователя</param>
        /// <returns></returns>
        internal mytrip_articlescategory CreateSubCategory(string title, int categoryId, bool allCulture,
            string path, string keywords, string desc,string seotitle)
        {
        ...
        /// <param name="culture">культура</param>
        /// <returns></returns>
        internal mytrip_articlescategory CreateBlog(string culture)
        {
            CreateСategoryZero();
            int categoryId = CreateCategoryId();
            ...
            mytrip_articlescategory x = new mytrip_articlescategory
            {
                ...
                AllCulture = false,
                Culture = culture,
                SeoDescription = ModuleSetting.BlogsPageDesc(),
                SeoKeyword = ModuleSetting.BlogsPageKeyWords(),
                SeoTitle=title
            };
            entities.mytrip_articlescategory.AddObject(x);
            entities.SaveChanges();
            GeneralMethods.MytripCacheRemove("as_getblogs",true,true);
            GeneralMethods.MytripCacheRemoveContains("as_getcategor");
            return x;
        }
        ...
        /// <param name="title">название</param>
        /// <param name="separateBlock">добавить в меню</param>
        internal void UpdateCategory(int categoryId, string title, bool separateBlock, bool allCulture,
            string path, string keywords, string desc,string seotitle)
        {
            ...
            c.Title = title;
            c.Path = (path != null && path.Length > 0) ? path : GeneralMethods.DecodingString(title);
            c.SeparateBlock = separateBlock;
            c.AllCulture = allCulture;
            c.SeoKeyword = keywords;
            c.SeoDescription = desc;
            c.SeoTitle = (seotitle != null && seotitle.Length > 0) ? seotitle : title;
            entities.SaveChanges();
            GeneralMethods.MytripCacheRemoveContains("as_getcategor");
            GeneralMethods.MytripCacheRemoveContains("as_getarticles");
        }
        internal void UpdateCategory(int categoryId, string userName, string userEmail,bool incSubCats)
        {
            mytrip_articlescategory c = GetCategory(categoryId);
            c.UserName = userName;
        ...
        /// <param name="categoryId">номер рубрики</param>
        /// <param name="title">название</param>
        internal void UpdateBlog(int categoryId, string title)
        {
        ...
        /// <summary>Удалить рубрику (удаляет все подрубрики и статьи)
        /// </summary>
        /// <param name="categoryId">номер рубрики</param>
        ...
        #region private
        private mytrip_articlescategory _GetCategory(int categoryId)
        {
            CreateСategoryZero();
            return entities.mytrip_articlescategory
        ...
        private IQueryable<mytrip_articlescategory> _GetCategories(bool forMenu, string culture)
        {
            var a = entities.mytrip_articlescategory
                //.Include("mytrip_articlescategory2")
                .Include("mytrip_articlescategory1")
        ...
        private IQueryable<mytrip_articlescategory> _GetBlogs(string culture)
        {
            return entities.mytrip_articlescategory
                .Where(x => x.Blog)
                .Where(x => x.SubCategoryId == 0)
        ...
        /// <summary>Создать рубрику ZERO
        /// </summary>
        private void CreateСategoryZero()
        {
