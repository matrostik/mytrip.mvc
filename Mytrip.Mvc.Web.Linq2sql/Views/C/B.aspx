...
<!--/*   Mytrip.Mvc.Web.Linq2sql   Copyright ©  2009 Стюхин Олег Анатольевич   */-->
 ...
 <!-- АДМИН ЧАСТЬ -->
    ...
    <%= Html.CreateCategory((bool)Model.News,(bool)Model.Blog, Model.CategoryId) %>
    <%if (Model.CategoryId == 0) {%>
    <%= Html.EditeAndDeliteCategory((bool)Model.Blog, Model.Id, Model.CategoryId, Model.AddedBy)%>
    <% }
      else
      { %>
    <%= Html.EditeAndDeliteCategory((bool)Model.Blog, Model.Id, Model.CategoryId, Model.AddedBy, Model.mt_artycle_category1.AddedBy)%>
    <%} %>    
    <%= Html.CreateReCategory((bool)Model.News,(bool)Model.Blog,Model.Id,Model.CategoryId,Model.AddedBy) %>  
    <%= Html.CreateArtycle(Model.Id,(bool)Model.News, (bool)Model.Blog, Model.AddedBy) %>
    </div>
    <!--END АДМИН ЧАСТЬ -->
    <!-- ЗАГОЛОВОК -->
    <%if (Model.CategoryId != 0)
