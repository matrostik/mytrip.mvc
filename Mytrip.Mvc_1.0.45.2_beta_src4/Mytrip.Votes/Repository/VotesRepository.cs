...
namespace Mytrip.Votes.Repository
{
    public class VotesRepository
    {
        ...
        VotesSetting _core;
        public VotesSetting core
        {
            get
            {
                if (_core == null)
                    _core = new VotesSetting();
                return _core;
            }
        }
        Entities _entities;
        ...
        public void OnOffQuestion(int questionId)
        {
            ...
            if (!question.Active)
            {
                ...
                {
                    var aq = entities.mytrip_votesquestion.FirstOrDefault(x => x.Active == true&&x.Culture==question.Culture);
                    if (aq != null)
                        aq.Active = false;
                }
            }
        ...
        {
            int id;
            for (id = 1; GetQuestion(id) != null; id++) ;
            return id;
        }
        private int CreateAnswerId()
        {
            int id;
            for (id = 1; GetAnswer(id) != null; id++) ;
            return id;
        }
        #endregion
    }
