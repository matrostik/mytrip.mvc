...
namespace Mytrip.Articles.Helpers
{
    public static class ArchiveHelpers
    {
        ...
        public static string ArchiveStatistic(this HtmlHelper html)
        {
            ...
            foreach (string culture in loc.allCultureMassive())
            {
            ...
            foreach (string culture in loc.allCultureMassive())
            {
                ...
                int count = ar.article.GetArticlesCount(culture);
            ...
            th51.MergeAttribute("colspan", loc.allCultureMassive().Count().ToString() + 2);
            ...
            foreach (string culture in loc.allCultureMassive())
            {
                ...
                int count = ar.article.GetPostsCount(culture);
            ...
            th101.MergeAttribute("colspan", loc.allCultureMassive().Count().ToString() + 2);
            ...
            td112.MergeAttribute("colspan", loc.allCultureMassive().Count().ToString());
            ...
            foreach (string culture in loc.allCultureMassive())
            {
                ...
                int count = ar.article.GetArticlesClosedCount(culture);
        ...
        public static string LatestUpdates(this HtmlHelper html, int count)
        {
            ...
            foreach (string culture in loc.allCultureMassive())
            {
                ...
                foreach (var tag in ar.article.GetAllTags(true))
                {
                    TagBuilder link = new TagBuilder("a");
                    link.MergeAttribute("href", LangLink(culture, "/Article/Index/1/10/" + tag.TagId + "/" + tag.Path));
                    link.InnerHtml = tag.TagName;
                    div.InnerHtml += " <b>" + link + "</b>(" + ar.article.GetArticlesInTagCount(culture,tag.TagId) + ") ";
                }
                td112.InnerHtml += div;
                ctr++;
            }
        ...
        public static string TagsDetails()
        {
            ...
            foreach (string culture in loc.allCultureMassive())
            {
                table.InnerHtml += BuildRow(culture, ar.article.GetAllTags(true));
            }
        ...
        static string BuildRow(string title, IQueryable<mytrip_articlescomments> comments)
        {
            ...
            foreach (var comment in comments)
            {
                ...
                link.MergeAttribute("href", LangLink(comment.mytrip_articles.Culture, "/Article/View/" + comment.mytrip_articles.ArticleId + "/" + comment.mytrip_articles.Path + "#" + comment.CommentId));
                link.InnerHtml = comment.mytrip_articles.Title;
                div.InnerHtml += EditDelete(comment) + comment.CreateDate.ToString("dd MMMM yyyy HH:mm:ss") + " "+ArticleLanguage.added_by+" " + ProfileLink(comment.UserName, comment.IsAnonym) + " in <b>" + link + " "
        ...
        static string BuildRow(string title, IQueryable<mytrip_articlestag> tags)
        {
            ...
            foreach (var tag in tags)
            {
                ...
                link.InnerHtml = tag.TagName;
                div.InnerHtml += EditDelete(tag) + "  <b>" + link + "</b>(" + ar.article.GetArticlesInTagCount(title.ToLower(),tag.TagId) + ")   ";
            }
            accordionContent2.InnerHtml += div;
            accordion2.InnerHtml = accordionTitle2.ToString() + accordionContent2.ToString();
            td.InnerHtml = accordion2.ToString();
