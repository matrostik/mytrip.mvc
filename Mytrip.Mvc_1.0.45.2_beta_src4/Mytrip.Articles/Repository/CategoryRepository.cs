...
namespace Mytrip.Articles.Repository
{
    public class CategoryRepository
    {
        ...
        /// <summary>
        /// Получить все рубрики статей (для меню и недобавленные в меню) с начала 1->X
        /// </summary>
        ...
        public mytrip_articlescategory CreateBlog(string culture)
        {
            CreateСategoryZero();
            int categoryId = CreateCategoryId();
            string title = username + "(" + ArticleLanguage.blog + ")";
            mytrip_articlescategory x = new mytrip_articlescategory
            {
                CategoryId = categoryId,
                Title = title,
        ...
        {
            var art1 = entities.mytrip_articles
                .Where(d => d.mytrip_articlescategory.SubCategoryId == categoryId);
            var art2 = entities.mytrip_articles
                .Where(d => d.mytrip_articlescategory.CategoryId == categoryId);
            List<int> ids = art1.Union(art2)
                .Select(h => h.ArticleId).ToList();
            foreach (int id in ids)
            {
                foreach (var v in entities.mytrip_articlesvotes.Where(df => df.ArticleId == id))
                {
                    entities.mytrip_articlesvotes.DeleteObject(v);
                }
                foreach (var v in entities.mytrip_articlescomments.Where(df => df.ArticleId == id))
                {
                    entities.mytrip_articlescomments.DeleteObject(v);
                }
                mytrip_articles article = entities.mytrip_articles.Include("mytrip_articlestag")
              .FirstOrDefault(x => x.ArticleId == id);
                foreach (var v in article.mytrip_articlestag.ToList())
                {
                    article.mytrip_articlestag.Remove(v);
                }
            ...
            entities.mytrip_articlescategory.DeleteObject(entities.mytrip_articlescategory.FirstOrDefault(j => j.CategoryId == categoryId));
