...
namespace Mytrip.Articles.Models
{
    ...
    public class ArticleViewModel
    {
        ...
        public string Path { get; set; }
        public string Title { get; set; }
        public bool replaceCommentsEmail { get; set; }
    ...
    public class RoleArticleAttribute : ActionFilterAttribute
    {
        ...
        {
            if (db.IsUserInRoleOnline(artset.roleArticleEditor()) || db.IsUserInRoleOnline(artset.roleBlogger()) || db.IsUserInRoleOnline(artset.roleChiefEditor()))
            {
    ...
    public class _CaptchaErrorAttribute : ValidationAttribute
    {
        ...
        public override bool IsValid(object value)
        {
            ...
            if (string.IsNullOrEmpty(captcha))
                return false;
            if (!HttpContext.Current.User.Identity.IsAuthenticated && db.mtHashCaptcha(captcha) != HttpContext.Current.Session["antibotimage"].ToString())
            {
                result = false;
            }
