    ...
    public static class DepartmentAndProducerHelpers
    {
        ...
        public static HtmlString StoreDepartment(this HtmlHelper html, IQueryable<mytrip_storedepartment> department, int take)
        {
            ...
            foreach (var article in department)
            {
                ...
                string _content = string.Format("<a href=\"/Store/Index/1/10/{0}/0/1/{1}\">{2}</a><b><a href=\"/Store/Index/1/10/{0}/0/1/{1}\" class=\"hometitle\" >{3} ({4})</a></b><br/>{5}{6}", article.DepartmentId, article.Path, StoreHelper.GetImage(article.DepartmentId, "/Content/Store/Department", ModuleSetting.widthImgDepartment()), article.Title, countproduct, article.Body, _subdepartment);
        ...
        public static HtmlString StoreTitleDepartmentAndProducer(this HtmlHelper html, TitleDepartmentModel x)
        {
            ...
            if (x.count >= 0)
            {
                string image = "";
                if (x.producer)
                    image = StoreHelper.GetImage(x.produceridforeditor, "/Content/Store/Producer", ModuleSetting.widthImgDepartment());
                else
                    image = StoreHelper.GetImage(x.createdepartment, "/Content/Store/Department", ModuleSetting.widthImgDepartment());
